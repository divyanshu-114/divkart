                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center space-x-2 mt-2">
                              <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth"><Minus className="w-4 h-4 text-foreground" /></button>
                              <span className="w-8 text-center font-semibold">{item.quantity}</span>
                              <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth"><Plus className="w-4 h-4 text-foreground" /></button>
                              <button onClick={() => dispatch(removeFromCartAPI(item.id))} className="p-1 rounded glass-card hover:glow-on-hover animate-smooth ml-2 text-destructive"><Trash2 className="w-4 h-4 text-destructive-foreground" /></button>
                            </div>
                          </div>
                        </div>
                      </div>
